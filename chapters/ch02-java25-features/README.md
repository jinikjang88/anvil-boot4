...
금융 메시징에서 결제 알림을 외부 PG / 은행망 / 알림 채널 3 곳에 동시 통보해야 했다. 처음 구현은 `RestTemplate` 동기 호출 셋을 줄세웠다. 각 200~400ms × 3 = 약 1초. TPS 200 이면 worker pool(20) 이 일주일에 한 번씩 포화 → 큐 적체 → 알림 지연.
